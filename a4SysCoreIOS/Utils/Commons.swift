...
class Commons{
    ...
    static func showMessage(_ content : String)
    {
//        let snackbar = TTGSnackbar.init(message: content, duration: .middle)
//        snackbar.show()
        /*
         ...
         _ = alert.showNotice("Push!", subTitle: content)*/
    }
...
//        }
    ...
    static func checkIfWifi() -> (Bool, String)
    {
        ...
        do {
            ...
            print("conoexion de internet : \(error.debugDescription)")
            return (false, "N")
        }
    }
    ...
    static func windowMessage(message: String, buttonTitle: String = "", action: Selector? = nil,action2: Selector? = nil, target: Any? = nil) {
        ...
        if !buttonTitle.isEmpty {
            ...
            button.backgroundColor = UIColor.hexStringToUIColor(hex: "#071051")
        ...
        if let selector = action2, let target = target{
            ...
            button2.backgroundColor = UIColor.hexStringToUIColor(hex: "#071051")
    ...
    static func windowInternetError(action: Selector? = nil, target: Any? = nil) {
        ...
        backgroundImage.frame = CGRect(x: 10, y: 20, width: Screen.width - 30.0, height: Screen.height - 50.0)
        view.insertSubview(backgroundImage, at: 0)
        
        if action != nil {
            let button = UIButton(type: .custom)
            button.frame = CGRect(x: Screen.midX - 150, y: Screen.midY + 200, width: 250, height: 100)
            button.backgroundColor = UIColor.hexStringToUIColor(hex: "#071051")
            button.layer.cornerRadius = 10.0
...
//                    NotificationCenter.default.post(.init(name: .AppActualizadaMessaging))
    ...
    static func activarTabBar(vc:UIViewController) {
        let tabbar = vc.tabBarController?.tabBar.items
        
        if let tabArray = tabbar {
            for a in tabArray {
                a.isEnabled = true
            }
            UIView.animate(withDuration: 0.3) { // se pone tranparencia al tab para dar a entender q estan bloqueados
                vc.tabBarController?.tabBar.alpha = 1
            }
        }
    }
}
